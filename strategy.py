...
======================================================================
Long-only strategy. Buy BTC at pullbacks; sell BTC we own on recovery.

Entry (trough detected):
  1. bb_pct < bb_entry_pct  → price at/below lower Bollinger band (1.5σ)
  2. RSI < rsi_entry         → oversold
  3. mom_slope > 0            → momentum ALREADY TURNING UP (knife-catcher filter)
  4. vol_ex < 4.0             → not a crash spike

Exit (mean-reversion complete):
  1. price >= bb_mid         → reached mid-band target
  2. RSI > rsi_exit          → overbought
  3. Stop: bb_lower - bb_stop_mult * bb_std (tight BB-extension stop)
  4. max_hold_bars            → anti-stall timer

R:R with bb_nstd=1.5: target=1.5σ, stop=0.5σ beyond lower band → 3:1
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Strategy — Long-only mean-reversion
# ─────────────────────────────────────────────────────────────────────────────

class MeanReversionMTF(Strategy):

    # ── Tunable parameters ─────────────────────────────────────────────────
    rsi_entry      = 42.0   # RSI oversold threshold
    rsi_exit       = 58.0   # RSI overbought threshold (exit long)
    bb_entry_pct   = 0.20   # Enter when price in lower bb_entry_pct of BB range
    bb_stop_mult   = 0.5    # Stop = bb_lower - bb_stop_mult * bb_std
    max_hold_bars  = 6      # 30s max hold → fast recycling
    taker_win_min  = 0.005
    ...
    # ── Exit: sell the BTC we own once mean-reversion completes ──────────
    ...
    # ── Stop: price fell further below lower band ─────────────────────────
    def _stop_hit(self) -> bool:
        return (not np.isnan(self._stop_price)) and self._close < self._stop_price
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self._should_enter():
                ...
                if np.isnan(stop_price) or stop_price >= entry_est: return

                n_units = max(1, int(self.equity * 0.95 / entry_est))
